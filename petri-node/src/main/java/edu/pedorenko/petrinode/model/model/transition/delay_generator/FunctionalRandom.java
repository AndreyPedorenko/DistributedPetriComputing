package edu.pedorenko.petrinode.model.model.transition.delay_generator;
...
class FunctionalRandom {
     ...
     */
    static double exp(double timeMean) {
        double a = 0;
        while (a == 0) {
            a = Math.random();
        }
     ...
     */
    static double unif(double timeMin, double timeMax) {
        double a = 0;
        while (a == 0) {
            a = Math.random();
        }
     ...
     */
    static double norm(double timeMean, double timeDeviation) {
        double a;
        Random r = new Random();
     ...
     */
    static double empiric(double[] x, double[] y) throws Exception {
        int n = x.length;
        if (y[n - 1] != 1.0)
